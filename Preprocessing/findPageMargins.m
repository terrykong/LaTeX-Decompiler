...
threshold = 5; %(1/median(y_projection))-(1/median(y_projection+5));
...
valid_peaks = y_peak_val>.3*median(y_peak_val) & y_peak_val<1.7*median(y_peak_val);
...
% make sure peaks differ from neighbors by at least 5 pixels
threshold = 5; %(1/median(x_projection))-(1/median(x_projection+5));
[x_peak_val,x_peak_loc] = findpeaks(x_projection,'Threshold',threshold);
% remove outliers -- most text lines will have same order of magnitude of
% pixels, so any peaks with too many or too few pixels should be tossed out
valid_peaks = x_peak_val>.3*median(x_peak_val) & x_peak_val<1.7*median(x_peak_val);
